...
/**
 * The single clock for the landing opening. Mounted at the layout level so every
 * participant — backdrop, name, hero copy, scroll cue — reads the SAME two gates instead
 * of each firing its own uncoordinated entrance.
 *
...
export function IntroProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    if (reduce) {
      // No bloom, no FOUT wait — settle everything at once (next frame, so the gate flip is
      // a subscription-style callback rather than a synchronous render cascade).
    ...
    // Mount → backdrop blooms. One frame after the "out" state paints, so the CSS transition
    // always has its starting frame and the bloom can never be batched away.
